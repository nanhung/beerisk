...
p1 <-ggplot(Dat1, aes(day, sur))+ scale_y_continuous(lim=c(0, 100))+
  ...
  stat_function(fun=function(x)100*exp(-0.191161*x), geom="line", color="gray10")+ geom_point(aes(shape = factor(dos)))+
  ylab("Survival (%)")+xlab(bquote('Day'))+
  theme(axis.text=element_text(size=12), axis.title=element_text(size=12))+
  labs(shape="Dose")+theme_bw()
...
Dat2 <- data.frame(mor.x, mor.y, morr.y.sd)
mod2 <- lm(mor.y~ mor.x)
summary(mod2)

## plot base + points
p2 <- ggplot(Dat2, aes(mor.x, mor.y)) + geom_point() + stat_smooth(method=lm, formula = y~x, color="gray10")+
